# SKU Target Safety Stock Feature Implementation

## 需求概述

新增功能以支援使用者在網頁介面輸入 SKU 層級的 Target Qty，並將其按店舖等級 (Class) 權重比例分配至各店鋪的 Safety Stock。

### 核心需求
1. **輸入方式**：在網頁介面（計算頁面）直接輸入每個 SKU 的 Target Qty。
2. **分配邏輯**：
   - 首先執行標準的 Safety Stock 計算。
   - 根據店舖等級 (Class) 的權重比例進行分配：
     - Class A (AA, A1, A2, A3)：權重 3
     - Class B (B1, B2)：權重 2
     - Class C (C1, C2)：權重 1
     - Class D (D1)：權重 1
   - 將使用者的 `SKU_Target_Qty` 按權重比例分配：
     `New_SS_i = Weight_i × (Target_Qty / Total_Weight)`
3. **優先順序**：Target Safety Stock 邏輯在標準計算之後執行，會覆蓋標準計算結果。

## 實施步驟

### 1. 更新 app.py UI
- 在檔案上傳後、計算按鈕前，新增 "SKU Target Qty Allocation" 區域。
- 使用 `st.data_editor` 顯示所有 SKU 列表，供使用者輸入 Target Qty。
- 收集使用者輸入的 `sku_targets` 字典。
- 更新說明文字，明確告知分配比例為 Class A : Class B : Class C : Class D = 3 : 2 : 1 : 1

### 2. 更新 app.py 計算邏輯
- 新增 `CLASS_WEIGHTS` 對照表，定義各 Class 的權重。
- 修改 `calculate_safety_stock` 函數，接收 `sku_targets` 參數。
- 執行標準計算流程，取得 `results_df`。
- 執行分配邏輯（按 Class 權重）：
  - 遍歷有設定 Target Qty 的 SKU。
  - 取得該 SKU 所在店舖的 Class 權重。
  - 計算總權重 `Total_Weight`。
  - 計算分配係數 `Factor = Target_Qty / Total_Weight`。
  - 初步分配（向下取整）：`Allocated_SS_i = floor(Weight_i × Factor)`。
  - 處理整數分配後的餘數（分配給小數部分最大的店鋪）。
  - 更新 `Target_Safety_Stock`（保留原 `Suggested_Safety_Stock`）。
  - 更新 `Target_Safety_Stock_Days` 和 `Notes`。
  - 設定 `Constraint_Applied` 為 `'Target Safety Stock'`。
  - 設定 `Calculation_Mode` 為 `'Target Safety Stock'`。

## 計算細節

### Class 權重對照表
| Class 類別 | 包含的 Class | 權重 (Weight) |
| :--- | :--- | :--- |
| **Class A** | AA, A1, A2, A3 | 3 |
| **Class B** | B1, B2 | 2 |
| **Class C** | C1, C2 | 1 |
| **Class D** | D1 | 1 |

### 分配公式
$$
Allocated\_SS_i = \lfloor Weight_i \times \frac{Target\_Qty}{\sum Weight} \rfloor
$$

### 餘數處理
$$
Remainder = Target\_Qty - \sum Allocated\_SS_i
$$
餘數將分配給 $(Weight_i \times Factor) - Allocated\_SS_i$ 最大（即小數部分最大）的前 $N$ 個店鋪。

## 欄位變更
- **Constraint_Applied**: 當觸發分配時，值為 `'Target Safety Stock'`。
- **Calculation_Mode**: 當觸發分配時，值為 `'Target Safety Stock'`。
- **Notes**: 新增分配詳情（Target Qty, Class, Weight, Total Weight, Allocation Factor, Allocated SS）。

## 實施狀態
- [x] 新增 `CLASS_CATEGORY_MAP` 對照表
- [x] 修改 `calculate_safety_stock` 函數以支援按 Class 權重分配
- [x] 更新 `config/settings.py` 以支援自訂權重
- [x] 更新側邊欄 UI 以支援自訂權重
- [x] 更新 UI 說明文字
- [x] 更新 Notes 以顯示分配詳情
- [x] 更新 README.md 文件
